const initializePageFlow = () => {
    ...
    // Helper function to serialize a DOM node into a structured segment array
    const serializeDomToSegments = (node) => {
        const segments = [];

        Array.from(node.childNodes).forEach(child => {
            if (child.nodeType === Node.TEXT_NODE) {
                const textContent = child.nodeValue;
                const wordsAndSpaces = textContent.split(/(\s+)/); // Split by whitespace, keeping whitespace
                ...
                wordsAndSpaces.forEach(part => {
                    if (part.length > 0) {
                        if (part.trim() === '') {
                            ...
                            segments.push({ type: 'word', value: part });
                        }
                    }
                });
            } else if (child.nodeType === Node.ELEMENT_NODE) {
                const elementSegment = {
                    type: 'element',
                ...
                for (const attr of child.attributes) {
                    elementSegment.attributes[attr.name] = attr.value;
                }
                ...
                if (child.style && child.style.length > 0) {
                    for (let i = 0; i < child.style.length; i++) {
                        ...
                        elementSegment.styles[prop] = child.style.getPropertyValue(prop);
                    }
                }

                // Recursively serialize children
                elementSegment.children = serializeDomToSegments(child);
    ...
    const createElementData = (element, pageNum) => {
        ...
        return {
            ...
            contentTree: serializeDomToSegments(element), // Store the structured content tree
    ...
    // Helper function to reconstruct DOM from structured segment array
    const reconstructSegmentsToDom = (segments, parentElement) => {
        segments.forEach(segment => {
            if (segment.type === 'whitespace') {
                parentElement.appendChild(document.createTextNode(segment.value));
            } else if (segment.type === 'word') {
                const span = document.createElement('span');
                span.classList.add('glyph-border'); // Apply glyph border here
                span.textContent = segment.value;
                parentElement.appendChild(span);
            } else if (segment.type === 'element') {
                const element = document.createElement(segment.tagName);

                // Apply attributes
                for (const attrName in segment.attributes) {
                    element.setAttribute(attrName, segment.attributes[attrName]);
                }

                // Apply inline styles
                for (const styleProp in segment.styles) {
                    element.style[styleProp] = segment.style[styleProp];
                }

                // Recursively reconstruct children
                reconstructSegmentsToDom(segment.children, element);
    ...
    const createElementFromData = (elementData) => {
        ...
        // Reconstruct content from the contentTree
        reconstructSegmentsToDom(elementData.contentTree, newElement);
        return newElement;
    };
    ...
    const calculateVirtualPage2Sum = () => {
        ...
        virtualPage2Elements.forEach((elementData, index) => {
            ...
            if (!estimatedHeight || estimatedHeight === 0) {
                // Estimate based on content length (simplified as we no longer have innerHTML string)
                // This estimation might need to be more sophisticated based on contentTree
                estimatedHeight = 35; // Default height for BR elements or small text
                if (elementData.tagName === 'h2') {
                    estimatedHeight = 40; // Estimated height for headings
                } else if (elementData.contentTree && elementData.contentTree.length > 0) {
                    // A very rough estimate based on number of segments
                    estimatedHeight = Math.max(20, Math.min(60, 20 + (elementData.contentTree.length / 5) * 10));
                }
            }
    ...
    const updateContentAndPositions = () => {
        ...
        pagebooks.forEach(pagebook => {
            ...
            Array.from(pagebook.children).forEach((child, index) => {
                ...
                const elementData = {
                    tagName: child.tagName.toLowerCase(),
                    attr_and_vals: attributes,
                    contentTree: serializeDomToSegments(child), // Use the new contentTree
        ...
        // No need to call applyGlyphBorders here, as it's handled during reconstruction
    };
